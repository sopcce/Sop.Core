...
namespace Sop.Data.Dapper
{
    ...
    public static class DapperExtensions
    {
            ...
            switch (dialect)
            {
                ...
                case Dialect.Oracle:
                    ...
                    *  http://stackoverflow.com/questions/5541455/implement-oracle-paging-for-any-query
                    ...
                    _encapsulation = "[{0}]";
                    _getIdentitySql = string.Format("SELECT CAST(@@IDENTITY AS BIGINT) AS[Id]");
                    _getPagedListSql = "SELECT * FROM(SELECT a.*, ROWNUM RNUM FROM(Select {SelectColumns} FROM {TableName} {WhereClause}) a WHERE ROWNUM <= {RowsPerPage}) b WHERE b.RNUM >= {PageNumber}";
                    break;
        ...
        public static async Task<IEnumerable<T>> GetPageListAsync<T>(this IDbConnection connection, int pageNumber, int rowsPerPage, string conditions, string orderby = null, object parameters = null, IDbTransaction transaction = null, int commandTimeout = 6000)
        {
            if (string.IsNullOrEmpty(_getPagedListSql))
                throw new Exception("GetListPage is not supported with the current SQL Dialect");
