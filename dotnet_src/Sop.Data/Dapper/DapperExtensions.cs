...
namespace Sop.Data.Dapper
{
    ...
    public static class DapperExtensions
    {
        ...
        {
            if (StringBuilderCacheEnabled && StringBuilderCacheDict.TryGetValue(cacheKey, out var value))
            {
                sb.Append(value);
                return;
            }
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                ...
                case Dialect.SqlCe:
                    _dialect = Dialect.SqlCe;
                    _encapsulation = "[{0}]";
                    _getIdentitySql = "SELECT CAST(@@IDENTITY AS BIGINT) AS[Id]";
                    _getPagedListSql = "OFFSET @firstResult ROWS FETCH NEXT @maxResults ROWS ONLY";
                    break;
                case Dialect.Oracle:
                    /* 
                    *  http://stackoverflow.com/questions/5541455/implement-oracle-paging-for-any-query
                        ...
                        {SELECT * FROM (
                       SELECT "_ss_dapper_1_".*, ROWNUM RNUM FROM (
                       sql) "_ss_dapper_1_"
                       WHERE ROWNUM <= :topLimit) "_ss_dapper_2_" 
                       WHERE "_ss_dapper_2_".RNUM > :toSkip
                       }
                     */
                    ...
                    _getIdentitySql = "SELECT CAST(@@IDENTITY AS BIGINT) AS[Id]";
                    _getPagedListSql =
                        "SELECT * FROM(SELECT a.*, ROWNUM RNUM FROM(Select {SelectColumns} FROM {TableName} {WhereClause}) a WHERE ROWNUM <= {RowsPerPage}) b WHERE b.RNUM >= {PageNumber}";
                    break;
            }
        }
        ...
        ///     <para>
        ///         orderby is a column or list of columns to order by ex: "lastname, age desc" - not required - default is by
        ///         primary key
        ///     </para>
        ///     <para>parameters is an anonymous type to pass in named parameter values: new { Age = 15 }</para>
        ///     <para>Supports transaction and command timeout</para>
                                                     ...
                                                     IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (string.IsNullOrEmpty(orderby)) orderby = GetColumnName(idProps.First());
        ...
        public static async Task<IEnumerable<T>> GetPageListAsync<T>(this IDbConnection connection, int pageNumber,
                                                                     int rowsPerPage, string conditions,
                                                                     string orderby = null, object parameters = null,
                                                                     ...
                                                                     int commandTimeout = 6000)
        {
            ...
            var recurrent = typeof(T);
            var idProps = GetIdProperties(recurrent).ToList();
            if (!idProps.Any())
                throw new ArgumentException("Entity must have at least one [Key] property");

            var name = GetTableName(recurrent);
            var sb = new StringBuilder();
            var query = _getPagedListSql;
            if (string.IsNullOrEmpty(orderby)) orderby = GetColumnName(idProps.First());
            ...
            if (Debugger.IsAttached)
                Trace.WriteLine($"GetListPaged<{recurrent}>: {query}");
        ...
        ///     <para>Supports transaction and command timeout</para>
        ///     <para>
        ///         Returns the ID (primary key) of the newly inserted record if it is identity using the int? type, otherwise
        ///         null
        ///     </para>
        /// </summary>
        /// <param name="connection"></param>
        ...
        ///     <para>Supports transaction and command timeout</para>
        ///     <para>
        ///         Returns the ID (primary key) of the newly inserted record if it is identity using the defined type, otherwise
        ///         null
        ///     </para>
        /// </summary>
        /// <param name="connection"></param>
                                                 ...
                                                 IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (keytype != typeof(int) && keytype != typeof(uint) && keytype != typeof(long) &&
                ...
                keytype != typeof(Guid) && keytype != typeof(string)) throw new Exception("Invalid return type");
            ...
            StringBuilderCache(masterSb, $"{props.CacheKey()}_BuildSelect", sb =>
            {
                ...
                for (var i = 0; i < propertyInfos.Count(); i++)
                {
                    ...
                    if (property.GetCustomAttributes(true)
                                .Any(attr => attr.GetType().Name == typeof(IgnoreSelectAttribute).Name ||
                    ...
                    if (sourceProperties.ElementAt(x).Name == propertyToUse.Name)
                    {
                        if (whereConditions != null && propertyToUse.CanRead &&
                            (propertyToUse.GetValue(whereConditions, null) == null ||
                             propertyToUse.GetValue(whereConditions, null) == DBNull.Value)) useIsNull = true;
            ...
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildInsertValues", sb =>
            {
                ...
                for (var i = 0; i < props.Count(); i++)
                {
                    ...
                    if (property.PropertyType != typeof(Guid) && property.PropertyType != typeof(string)
                                                              ...
                                                              && property.GetCustomAttributes(true)
                                                                         .All(attr => attr.GetType().Name !=
                                                                                      typeof(RequiredAttribute).Name))
                        continue;
                    if (property.GetCustomAttributes(true).Any(attr =>
                                                                   attr.GetType().Name ==
                                                                   typeof(IgnoreInsertAttribute).Name ||
                                                                   attr.GetType().Name ==
                                                                   typeof(NotMappedAttribute).Name ||
                                                                   attr.GetType().Name ==
                                                                   typeof(ReadOnlyAttribute).Name &&
                                                                   IsReadOnly(property))
                    )
                        continue;
        ...
        private static IEnumerable<PropertyInfo> GetScaffoldableProperties<T>()
        {
            ...
            props = props.Where(p => p.GetCustomAttributes(true)
                                      .Any(attr => attr.GetType().Name == typeof(EditableAttribute).Name &&
                                                   !IsEditable(p)) == false);
        ...
        private static bool IsReadOnly(PropertyInfo pi)
        {
            ...
            if (attributes.Length > 0)
            {
                ...
                if (write != null) return write.IsReadOnly;
            }
        ...
        private static IEnumerable<PropertyInfo> GetUpdateableProperties<T>(T entity)
        {
            ...
            updateableProperties = updateableProperties.Where(
                p => p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(KeyAttribute).Name) ==
                     false);
            //remove ones that are readonly
            updateableProperties = updateableProperties.Where(
                p => p.GetCustomAttributes(true)
                      .Any(attr => attr.GetType().Name == typeof(ReadOnlyAttribute).Name && IsReadOnly(p)) == false);
            //remove ones with IgnoreUpdate attribute
            updateableProperties = updateableProperties.Where(
                p => p.GetCustomAttributes(true)
                      .Any(attr => attr.GetType().Name == typeof(IgnoreUpdateAttribute).Name) == false);
            //remove ones that are not mapped
            updateableProperties = updateableProperties.Where(
                p => p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(NotMappedAttribute).Name) ==
                     false);
        ...
        private static IEnumerable<PropertyInfo> GetIdProperties(Type type)
        {
            var tp = type.GetProperties()
                         .Where(p => p.GetCustomAttributes(true)
                                      .Any(attr => attr.GetType().Name == typeof(KeyAttribute).Name)).ToList();
            return tp.Any()
                ? tp
        ...
        public class TableNameResolver : ITableNameResolver
        {
            public virtual string ResolveTableName(Type type)
            {
                ...
                var tableattr = type.GetCustomAttributes(true)
                                    .SingleOrDefault(attr => attr.GetType().Name == typeof(TableAttribute).Name) as
                    dynamic;
                if (tableattr != null)
                {
        ...
        public class ColumnNameResolver : IColumnNameResolver
        {
            public virtual string ResolveColumnName(PropertyInfo propertyInfo)
            {
                ...
                var columnattr = propertyInfo.GetCustomAttributes(true)
                                             .SingleOrDefault(
                                                  attr => attr.GetType().Name == typeof(ColumnAttribute).Name) as
                    dynamic;
                if (columnattr != null)
                {
    ...
    ///     You can use the System.ComponentModel.DataAnnotations version in its place to specify the properties that are
    ///     editable
    /// </summary>
    [AttributeUsage(AttributeTargets.Property)]
