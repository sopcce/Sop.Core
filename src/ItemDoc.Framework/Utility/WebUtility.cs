...
namespace ItemDoc.Framework.Utility
{
  ...
  public static class WebUtility
  {
    ...
    /// </summary>
    public static readonly string HttpUserAgent = "Mozilla/5.0 (Windows NT 6.1; Trident/7.0; rv:11.0; Alexa Toolbar) like Gecko";
    ...
    public static string GetParentUrlPath(string UrlPath, string lockInBaseUrl)
    {
    ...
    public static string GetCurrentDomain(bool MustRaw)
    {
    ...
    public static string GetBaseDomain(string FullDomain)
    {
    ...
    public static string GetDomainFromUrl(string fullUrl)
    {
    ...
    public static string SetResolveUrl(string relativeUrl)
    {
    ...
    public static string SetCompleteUrl(string content)
    {
    ...
    public static NameValueCollection GetCookies(string sKey)
    {
      ...
      if (httpCookie != null)
      {
        ...
        for (int i = 0; i < allKeys.Length; i++)
        {
          ...
          nameValueCollection.Add(name, HttpUtility.UrlDecode(httpCookie.Values[name], Encoding.GetEncoding(936)));
        }
      }
    ...
    public static void SetCookie(string sKey, string sValue, DateTime expiresTime)
    {
    ...
    /// <param name="sValue">要保存的值</param>
    /// <param name="expiresTime">到期时间，小于或等于当前时间的为临时Cookie。</param>
    public static void SetCookie(HttpResponse response, string sKey, string sValue, DateTime expiresTime)
    {
      if (string.IsNullOrEmpty(sKey))
      {
        return;
      ...
      httpCookie.Value = HttpUtility.UrlEncode(sValue, Encoding.Default);
    ...
    /// <param name="expiresTime">到期时间，小于或等于当前时间的为临时Cookie。</param>
    public static void SetCookies(string sKey, NameValueCollection sValues, DateTime expiresTime)
    {
    ...
    /// <param name="sValues">要存储的键/值对集合</param>
    /// <param name="expiresTime">到期时间，小于或等于当前时间的为临时Cookie。</param>
    public static void SetCookies(HttpResponse response, string sKey, NameValueCollection sValues, DateTime expiresTime)
    {
      ...
      {
        string name = allKeys[i];
        httpCookie.Values[name] = HttpUtility.UrlEncode(sValues[name], Encoding.GetEncoding(936));
      }
      httpCookie.HttpOnly = true;
      if (expiresTime > DateTime.Now)
      {
    ...
    public static bool IsComeFromSearchEngine()
    {
      ...
      return (text.Contains(".baidu.com") || text.Contains(".soso.com") || text.Contains(".youdao.com") || text.Contains(".yahoo.com") || text.Contains(".sogou.com") || text.Contains(".bing.com") || text.Contains(".so.com") || text.Contains(".google.com")) && text.Contains("?") && text.Contains("=");
    }
    ...
    {
      string text = KeepHtmlTag ? SrcText : HttpUtility.HtmlEncode(SrcText);
    ...
    public static bool IsContainsHtmlTranslationCharacters(string SrcValue)
    {
      ...
      string text = HttpUtility.HtmlEncode(SrcValue);
