    ...
    public class IdSnowflake
  {
    ...
    private void Snowflakes(long machineId, long datacenterId)
    {
      if (machineId >= 0)
      {
        ...
        IdSnowflake.machineId = machineId;
      }
      if (datacenterId >= 0)
      {
        ...
        IdSnowflake.datacenterId = datacenterId;
      }
    }
    ...
    public long GetId()
    {
      ...
      {
        long timestamp = GetTimestamp();
        if (IdSnowflake.lastTimestamp == timestamp)
        { //同一微妙中生成ID
          sequence = (sequence + 1) & sequenceMask; //用&运算计算该微秒内产生的计数是否已经到达上限
          if (sequence == 0)
          {
            //一微妙内产生的ID计数已达上限，等待下一微妙
            timestamp = GetNextTimestamp(IdSnowflake.lastTimestamp);
          }
        }
        else
        {
        ...
        IdSnowflake.lastTimestamp = timestamp; //把当前时间戳保存为最后生成ID的时间戳
