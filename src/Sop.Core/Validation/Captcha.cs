...
namespace Sop.Core.Validation
{
    ...
    public class Captcha
    {
        ...
        public Captcha()
        {
            _validateCodeCount = Config.AppSettings("Captcha.CodeCount", _validateCodeCount);
            _validateCodeCount = _validateCodeCount != 4 || _validateCodeCount < 16 ? 4 : _validateCodeCount;

            string allChar = Config.AppSettings("Captcha.Code", AllChar);
            AllChar = allChar.Length > 7 ? allChar : AllChar;
        }
        #region 私有变量/函数方法
        /// <summary>
        /// 验证码存储时用的Key值。
        ...
        public static Stream GetImageGif()
        {
            CreateValidate();
        ...
        public static byte[] SetByteValidate()
        {
            //把生成的验证码
            CreateValidate();
            var memoryStream = StreamToMemoryStream(GetImageGif());
            byte[] asd = memoryStream.ToArray();
            //保存到cookie中
        ...
        public static Stream SetStreamValidate()
        {
            //把生成的验证码输入到cookie
            CreateValidate();
            Stream ms = GetImageGif();
            //保存到cookie中
            Captcha.SaveCheckCode(ValidateCode);
            return ms;
        }

        public static MemoryStream SetMemoryStreamValidate()
        {
            //把生成的验证码输入到cookie
            CreateValidate();
            var ms = StreamToMemoryStream(GetImageGif());
            //保存到cookie中
            Captcha.SaveCheckCode(ValidateCode);
            return ms;
        }
