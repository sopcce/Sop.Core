...
namespace Sop.Core.WebUtility
{
    ...
    public class UrlUtility
    {
        ...
        /// <param name="relativeUrl">相对url</param>
        public string SetFixUrl(string baseUrl, string relativeUrl)
        {
            relativeUrl = Regex.Replace(relativeUrl, "(?is)(href|src)=(\"|\')([^(\"|\')]+)(\"|\')", (match) =>
            {  
                ...
                try
                {
                    ...
                    string fullUrl = $"{match.Groups[1].Value}=\"{thisUri.AbsoluteUri}\"";
                    return fullUrl;
                }
                catch (Exception)
                {
